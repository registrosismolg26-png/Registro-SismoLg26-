...
- **Roles:** MASTER (global, se gestiona por SQL, no asignable en la UI), ADMIN (su refugio), REGISTRADOR (censa su refugio), VISUALIZADOR (solo ve). **Solo Master asigna Admin; nadie asigna Master.** **Roles médicos** (`isMedico`: AdminMedico, OperadorMedico, AsistenteMedico) **solo ven Morbilidad**; AdminMedico además ve Usuarios FILTRADO a médicos de su refugio (crea Operador/Asistente; crear AdminMedico es solo de Master). OperadorMedico crea/edita consultas y catálogos (no elimina); AsistenteMedico solo consultas. Consultas EDITABLES (botón + modal en historial; POST hace upsert-update; nadie elimina). Médicos también ven **Balance de Salud** (`BalanceTab`).
...
- **Botoneras AGRUPADAS (regla de tema visual innegociable).** Los botones de un mismo set de acciones van **agrupados**, no sueltos. Patrones reutilizables en `globals.css`: **grupo segmentado** (`.btn-seg-group`, envuelve `.toolbar-btn` en un pill único con divisores; texto colapsa a ícono ≤560px con `.btn-txt-collapsible`) para toolbars; y **grupo de íconos con tinte por acción** (`.row-actions` + `.btn-ver` cuadrado redondeado + `--room` verde / `--view` azul / `--edit` ámbar) para acciones de fila. Usa/extiende estos patrones; nunca dejes un set de botones desalineado o con estilos dispares. **Verifícalo visualmente.**

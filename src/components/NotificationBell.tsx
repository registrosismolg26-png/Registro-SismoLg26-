  ...
  const paths: Record<string, ReactNode> = {
    AVISO: (
      <>
        ...
        <path d="M11 16.5a2.5 2.5 0 0 1-4.9-.5" />
      </>
    ),
    USUARIO_NUEVO: (
      <>
...
function accionDe(
  n: Notif,
  role: string,
): { label: string; can: boolean } | null {
  if (n.tipo === "TRASLADO" && n.entidadId)
    return { label: "Ver ficha", can: !isMedico(role) };
  if (n.tipo === "USUARIO_NUEVO" && n.entidadId)
    return { label: "Ver usuario", can: canManageUsers(role) };
  if (n.tipo === "BIENVENIDA")
    return {
      label: "Ir a Configuración",
      can: !isMedico(role) && role !== "VISUALIZADOR",
    };
  return null;
}
...
export default function NotificationBell() {
  ...
  const doHide = (id: string, mode: "dismiss" | "snooze") =>
    setHidden((h) => hideNotif(h, id, mode));
  ...
  const loadAvisos = async (force = false) => {
    ...
    if (cacheAvisos && Date.now() - cacheAvisos.at < (force ? 2_000 : 45_000)) {
      setItems(cacheAvisos.items);
      setUnread(cacheAvisos.unread);
      setNuevos(cacheAvisos.nuevos);
      return;
    }
    try {
      const q = esAdminCenso
        ? `?afectadosSince=${encodeURIComponent(seenSince() || "")}`
        : "";
  ...
  const loadAfectados = async () => {
    ...
    try {
      ...
      const res = await apiFetch(
        `/api/nuevos-afectados${prev ? `?since=${encodeURIComponent(prev)}` : ""}`,
      );
      const d = await res.json().catch(() => ({}));
  ...
  const esNuevoAfectado = (r: Afectado) => {
    ...
    return prev
      ? new Date(r.createdAt).getTime() > new Date(prev).getTime()
      : false;
  };
  ...
  const visibleAfectados = afectados.filter(
    (r) => !isHiddenNow(hidden, "afec:" + r.id),
  );
    ...
    <>
      {esAdminCenso ? (
        <div className="notif-seg">
          <button
            type="button"
            className={`notif-seg__btn${section === "avisos" ? " is-active" : ""}`}
            onClick={() => irASeccion("avisos")}
          >
            Avisos
            {unread > 0 && (
                ...
                >
                  <span
                    ...
                    style={{
                      ...
                      background: `${tipoColor(n.tipo)}22`,
                    }}
                  >
                    ...
                    <span className="notif-item__title">
                      ...
                      {!n.readAt && (
                        <span
                          ...
                          style={{ background: tipoColor(n.tipo) }}
                        />
                      )}
                    </span>
              ...
              >
                ...
                <span className="notif-item__main">
                  <span className="notif-item__title">
                    {r.nombreApellido}
                    {esNuevoAfectado(r) && (
                      <span className="afec-badge">Nuevo</span>
                    )}
                    {r.retirado === "SI" && (
                      <span className="afec-badge afec-badge--out">
                        Retirado
                      </span>
                    )}
                  </span>
                  <span className="notif-item__body">
                    C.I. {r.cedula} · {r.refugio}
  ...
  return (
    <div className="notif-bell">
      ...
      >
        ...
        {badge > 0 && (
          <span className="notif-bell__badge">{badge > 9 ? "9+" : badge}</span>
        )}
      </button>
      ...
      {detail &&
        ...
        createPortal(
          ...
          >
            <div
              className="modal-content notif-detail"
            ...
            >
              <div className="notif-detail__head">
                <span
                  className="notif-item__ico"
                  style={{
                    color: tipoColor(detail.tipo),
                    background: `${tipoColor(detail.tipo)}22`,
                  }}
                >
                  <TipoIcon tipo={detail.tipo} />
                </span>
              ...
              <p
                className="notif-detail__body"
                style={{ whiteSpace: "normal" }}
              >
                <WaText text={detail.cuerpo} />
              </p>
              <p className="notif-detail__time">
                {fechaLarga(detail.createdAt)}
              </p>
              {accion && accion.can && (
